...
async function handleShowChunks(doc: DocumentItem) {
  chunksDocName.value = doc.fileName
  chunksVisible.value = true
  chunksLoading.value = true
  try {
    chunksData.value = await fetchDocumentChunks(doc.documentId)
  } catch {
    chunksData.value = []
  } finally {
...
const hasActiveDocuments = computed(() =>
  documents.value.some((d) => d.status === 'UPLOADED' || d.status === 'PROCESSING' || d.status === 'PENDING'),
)

let pollTimer: ReturnType<typeof setInterval> | null = null

watch(hasActiveDocuments, (active) => {
...
<template>
  <div class="doc-center">
    ...
    <!-- Chunks modal (teleported to body to avoid stacking context issues) -->
    <Teleport to="body">
      <div v-if="chunksVisible" class="chunks-overlay" @click.self="chunksVisible = false">
        <div class="chunks-modal">
          <div class="chunks-modal__head">
            <h3>分块列表 — {{ chunksDocName }}</h3>
            <span class="chunks-modal__count">共 {{ chunksData.length }} 块</span>
            <button class="chunks-modal__close" @click="chunksVisible = false">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
            </button>
          </div>
          <div class="chunks-modal__body">
            <div v-if="chunksLoading" class="chunks-modal__loading">加载中...</div>
            <div v-else-if="chunksData.length === 0" class="chunks-modal__empty">暂无分块数据</div>
            <div v-else v-for="c in chunksData" :key="c.chunkId" class="chunk-card">
              <div class="chunk-card__index">#{{ c.chunkIndex + 1 }}</div>
              <div class="chunk-card__text">{{ c.chunkText }}</div>
              <div class="chunk-card__meta">{{ c.chunkText.length }} 字符 · 位置 {{ c.charStart }}-{{ c.charEnd }}</div>
            </div>
          </div>
        </div>
      </div>
