 ...
 */
export async function fetchDocumentChunks(documentId: number): Promise<DocumentChunk[]> {
  const { data } = await http.get<ApiResponse<DocumentChunk[]>>(`/documents/${documentId}/chunks`)
  if (!data.success || !data.data) {
    throw new Error(data.message ?? '加载分块数据失败')
  }
  return data.data
}
...
export async function uploadDocument(payload: UploadDocumentPayload): Promise<UploadResult> {
  const formData = new FormData()
  formData.append('groupId', String(payload.groupId))
  formData.append('file', payload.file)

  const { data } = await http.post<ApiResponse<UploadResult>>('/documents/upload', formData, {
